...
#define AW882XX_DRIVER_VERSION "V1.15.0.26"
			...
			struct snd_soc_dai *dai)
{
	...
	aw_dev_get_init_data(aw882xx->aw_pa, &g_init_data);
			...
			unsigned int size, unsigned int __user *tlv)
{
	...
	case SNDRV_CTL_TLV_OP_READ:
		...
		if (copy_to_user((void __user *)(tlv + AW_TLV_HEADER_SIZE), (char *)&algo_data, sizeof(struct algo_auth_data)))
			ret = -EFAULT;
		break;
	case SNDRV_CTL_TLV_OP_WRITE:
		aw_dev_dbg(aw882xx->dev, "set algo auth data");
		if (copy_from_user(&algo_data, (void __user *)(tlv + AW_TLV_HEADER_SIZE), sizeof(struct algo_auth_data)))
			ret = -EFAULT;
