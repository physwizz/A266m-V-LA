...
/* 9bit signed int val */
static int aw_pid_2308_get_voltage_offset(struct aw_device *aw_dev,
														int32_t *offset) {
	int ret = 0;
	signed char val = -1;
	int32_t temp = -1;
	...
	ret = aw882xx_i2c_read(aw882xx, AW_PID_2308_EFRH2_REG, &reg_value);

	val = (signed char)(reg_value & 0x00ff);
	temp = val;
	*offset = temp << 1;
	aw_dev_info(aw_dev->dev, "get reg_val 0x%x vol offset val %d", reg_value, *offset);
	return ret;
}
...
static int aw_pid_2308_dev_init(struct aw882xx *aw882xx)
{
	...
	aw_pa->i2sen_desc.reg = AW_PID_2308_SYSCTRL_REG;
	aw_pa->i2sen_desc.i2sen_check = AW_PID_2308_I2SEN_CHECK;
