				...
				struct ae_param *expo)
{
	...
	/* get updated min expo time */
	v4l2_ctrl.id = V4L2_CID_SENSOR_GET_MIN_EXPOSURE_TIME;
	v4l2_ctrl.value = -1;
	ret = is_sensor_g_ctrl(device, &v4l2_ctrl);
	if (ret < 0) {
		err("get min_exposure_time fail");
	}
	min_expo_time = (u32)v4l2_ctrl.value;
	...
	/* expo clipping */
	if (min_expo_time > 0) {
		if (expo->long_val != 0 && min_expo_time > expo->long_val)
			expo->long_val = min_expo_time;
		if (expo->middle_val != 0 && min_expo_time > expo->middle_val)
			expo->middle_val = min_expo_time;
		if (expo->short_val != 0 && min_expo_time > expo->short_val)
			expo->short_val = min_expo_time;
	}

	dbg_sensor(1, "min_expo_time(%d) expo(%d/%d/%d)",
				min_expo_time, expo->long_val, expo->middle_val, expo->short_val);
...
void is_sensor_ctl_frame_evt(struct is_device_sensor *device)
{
	...
	if ((module_ctl->valid_sensor_ctrl == true) ||
		...
		ret = is_sensor_peri_update_seamless_mode(device);
		if (ret < 0)
			err("[%s] frame number(%d) update seamless mode fail\n", __func__, applied_frame_number);
