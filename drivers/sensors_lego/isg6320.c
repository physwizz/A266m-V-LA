...
static ssize_t isg6320_register_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct isg6320_data *data = dev_get_drvdata(dev);
	char *p = buf;
	...
	for (idx = 0; idx < data->reg_size; idx++) {
		isg6320_i2c_read(data, data->setup_reg[idx * 2], &val, 1);
...
static ssize_t isg6320_register_write_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t count)
{
	struct isg6320_data *data = dev_get_drvdata(dev);
	...
	isg6320_i2c_write(data, (unsigned char)reg_address, (unsigned char)val);
...
static DEVICE_ATTR(register_read_all, 0444, isg6320_register_show, NULL);
...
static DEVICE_ATTR(register_write, 0220, NULL, isg6320_register_write_store);
...
static int isg6320_parse_dt(struct isg6320_data *data, struct device *dev)
{
	...
	if (data->ic_num == MAIN_GRIP)
		data->gpio_int = of_get_named_gpio(node, "isg6320,irq-gpio", 0);
#if defined(CONFIG_SENSORS_ISG6320_SUB)
	else if (data->ic_num == SUB_GRIP)
		data->gpio_int = of_get_named_gpio(node, "isg6320_sub,irq-gpio", 0);
#endif
#if defined(CONFIG_SENSORS_ISG6320_WIFI)
	else if (data->ic_num == WIFI_GRIP)
		data->gpio_int = of_get_named_gpio(node, "isg6320_wifi,irq-gpio", 0);
#endif
#if defined(CONFIG_SENSORS_ISG6320_SUB2)
	else if (data->ic_num == SUB2_GRIP)
		data->gpio_int = of_get_named_gpio(node, "isg6320_sub2,irq-gpio", 0);
#endif
	...
	if (data->ic_num == MAIN_GRIP)
		...
		ret = of_property_read_u32(node, "isg6320_sub2,reg_num", &data->reg_size);
#endif
	...
	if (data->ic_num == MAIN_GRIP)
		...
		ret = of_property_read_u8_array(node, "isg6320_sub2,set_reg", data->setup_reg,
						data->reg_size * 2);
#endif
	if (ret < 0) {
	...
	if (data->ic_num == MAIN_GRIP)
		...
		ret = of_property_read_u32(node, "isg6320_sub2,multi_use", &data->multi_use);
#endif
	if (ret < 0) {
...
static int isg6320_check_dependency(struct device *dev, int ic_num)
{
	...
	gpio_ldo = of_get_named_gpio(dNode, "isg6320,ldo_en", 0);
	if (gpio_ldo >= 0) {
		if (!(vdd_retry_cnt % 50)) {
			pr_info("GRIP[%d] ldo rety cnt : %d\n", ic_num, vdd_retry_cnt);

			ret = gpio_request(gpio_ldo, "grip_ldo_en");
			if (ret < 0) {
				pr_info("GRIP[%d] gpio %d request failed %d\n", ic_num, gpio_ldo, ret);
				return ret;
			}

			gpio_direction_output(gpio_ldo, 1);
			pr_info("GRIP[%d] ldo_en power %d\n", ic_num, gpio_get_value(gpio_ldo));
			gpio_free(gpio_ldo);
		}
	...
	if (of_property_read_string_index(dNode, "isg6320,dvdd_vreg_name", 0, (const char **)&dvdd_vreg_name))
		dvdd_vreg_name = NULL;

	if (dvdd_vreg_name) {
		if (!(vdd_retry_cnt % 50))
			pr_info("GRIP[%d] dvdd_vreg_name %s (%d)\n", ic_num, dvdd_vreg_name, vdd_retry_cnt);
...
#endif
#if defined(CONFIG_SENSORS_ISG6320_SUB2)
	else if (strcmp(client->name, "isg6320_sub2") == 0)
		ic_num = SUB2_GRIP;
#endif
	else
		goto err;

	ret = isg6320_check_dependency(&client->dev, ic_num);
	if (ret < 0)
		return ret;
...
static void isg6320_remove(struct i2c_client *client)
#else
...
#if defined(CONFIG_SENSORS_ISG6320_SUB2)
static const struct of_device_id isg6320_sub2_match_table[] = {
	{ .compatible = "isg6320_sub2", },
	{ },
};

static struct i2c_device_id isg6320_sub2_id_table[] = {
	{"ISG6320_SUB2", 0},
	{ },
};
MODULE_DEVICE_TABLE(i2c, isg6320_sub2_id_table);

static struct i2c_driver isg6320_sub2_driver = {
	.driver = {
		.name = isg6320_sub2_id_table[0].name,
		.owner = THIS_MODULE,
		.of_match_table = isg6320_sub2_match_table,
		.pm = &isg6320_pm_ops,
	},
	.id_table = isg6320_sub2_id_table,
	.probe = isg6320_probe,
	.remove = isg6320_remove,
	.shutdown = isg6320_shutdown,
};
...
#endif
#if defined(CONFIG_SENSORS_ISG6320_SUB2)
	pr_err("defined config sensor sub2 \n");
	ret = i2c_add_driver(&isg6320_sub2_driver);
	if (ret != 0)
		pr_err("isg6320_sub2_driver probe fail\n");
#endif
	return ret;
}
...
#endif
#if defined(CONFIG_SENSORS_ISG6320_SUB2)
	i2c_del_driver(&isg6320_sub2_driver);
#endif
}
