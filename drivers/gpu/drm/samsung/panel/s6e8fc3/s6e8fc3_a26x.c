...
static int s6e8fc3_a26x_br_index_property_update(struct panel_property *prop)
{
...
static int s6e8fc3_a26x_first_br_property_update(struct panel_property *prop)
{
	struct panel_device *panel = prop->panel;
	struct panel_bl_device *panel_bl = &panel->panel_bl;
	...
	if (atomic_read(&panel_bl->props.brightness_non_zero_set_count) == 2)
		index = S6E8FC3_A26X_FIRST_BR_ON;
	else
		index = S6E8FC3_A26X_FIRST_BR_OFF;
...
static struct panel_prop_enum_item s6e8fc3_a26x_first_br_enum_items[MAX_S6E8FC3_A26X_FIRST_BR] = {
	__PANEL_PROPERTY_ENUM_ITEM_INITIALIZER(S6E8FC3_A26X_FIRST_BR_OFF),
	__PANEL_PROPERTY_ENUM_ITEM_INITIALIZER(S6E8FC3_A26X_FIRST_BR_ON),
};

static struct panel_prop_list s6e8fc3_a26x_property_array[] = {
	/* range property */
	__DIMEN_PROPERTY_RANGE_INITIALIZER(S6E8FC3_A26X_BR_INDEX_PROPERTY,
			0, 0, S6E8FC3_A26X_TOTAL_NR_LUMINANCE,
			s6e8fc3_a26x_br_index_property_update),
	__DIMEN_PROPERTY_ENUM_INITIALIZER(S6E8FC3_A26X_FIRST_BR_PROPERTY,
			S6E8FC3_A26X_FIRST_BR_OFF, s6e8fc3_a26x_first_br_enum_items,
			s6e8fc3_a26x_first_br_property_update),

#ifdef CONFIG_USDM_MDNIE
	__DIMEN_PROPERTY_RANGE_INITIALIZER(OLED_MDNIE_NIGHT_LEVEL_PROPERTY,
			0, 0, S6E8FC3_A26X_MAX_NIGHT_LEVEL,
			oled_mdnie_night_level_property_update),
#endif
};

static struct dump_expect s6e8fc3_a26x_self_mask_crc_expects[] = {
	{ .offset = 0, .mask = 0xFF, .value = 0x35, .msg = "Self Mask CRC[0] Error(NG)" },
	{ .offset = 1, .mask = 0xFF, .value = 0xf6, .msg = "Self Mask CRC[1] Error(NG)" },
};
static struct dumpinfo s6e8fc3_a26x_dump_self_mask_crc = DUMPINFO_INIT_V2(self_mask_crc,
		&s6e8fc3_restbl[RES_SELF_MASK_CRC], &OLED_FUNC(OLED_DUMP_SHOW_EXPECTS), s6e8fc3_a26x_self_mask_crc_expects);

static struct dump_expect s6e8fc3_a26x_self_mask_checksum_expects[] = {
...
static struct dumpinfo s6e8fc3_a26x_dump_self_mask_checksum = DUMPINFO_INIT_V2(self_mask_checksum,
		&s6e8fc3_restbl[RES_SELF_MASK_CHECKSUM], &OLED_FUNC(OLED_DUMP_SHOW_EXPECTS), s6e8fc3_a26x_self_mask_checksum_expects);
...
static int __init s6e8fc3_a26x_panel_init(void)
{
	...
	memcpy(&cpi->dumpinfo[DUMP_SELF_MASK_CRC],
		&s6e8fc3_a26x_dump_self_mask_crc, sizeof(struct dumpinfo));
...
module_exit(s6e8fc3_a26x_panel_exit);
