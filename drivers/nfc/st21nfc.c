...
static void st21nfc_set_i2c_pinctrl(struct device *dev, char *pinctrl_name)
{
	...
	pinctrl = devm_pinctrl_get_select(&i2c_pdev->dev, pinctrl_name);
	if (IS_ERR_OR_NULL(pinctrl))
		NFC_LOG_ERR("skip pinctrl %s\n", pinctrl_name);
	else
		devm_pinctrl_put(pinctrl);
	NFC_LOG_ERR("%s pinctrl set\n", pinctrl_name);
}
...
static int secnfc_init(struct i2c_client *client)
{
	...
	st21nfc_set_i2c_pinctrl(&client->dev, "i2c_pull_up");
...
static int st21nfc_nc_pinctrl(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(pinctrl))
		NFC_LOG_ERR("Failed to pinctrl nfc_nc %ld\n", PTR_ERR(pinctrl));
	else
		devm_pinctrl_put(pinctrl);
	...
	if (!IS_ERR_OR_NULL(i2c_dev_node)) {
		...
		if (i2c_pdev) {
			...
			if (IS_ERR_OR_NULL(pinctrl_i2c)) {
				NFC_LOG_ERR("No nfc_i2c_nc pinctrl %ld\n", PTR_ERR(pinctrl_i2c));
			} else {
...
#if IS_MODULE(CONFIG_SAMSUNG_NFC) && IS_ENABLED(CONFIG_NFC_ST54SPI_ESE_SUPPORT)
